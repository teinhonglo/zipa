...
gpuid=1
seed=66 #165, 33
train_conf=conf/train_l2arctic_baseline_wav2vec2_large_lv60_prompt.json
suffix=
...
exp_root=exp/l2arctic+timit/${conf_tag}${suffix}

if [ $stage -le 0 ]; then

    echo -e "${GREEN}Stage 0: data preparation ...${NC}"

    local/l2arctic/data_prep.sh --stage 0 \
...
if [ $stage -le 3 ]&& [ $stop_stage -ge 3 ]; then
    ...
    for test in $tests; do
        ...
        eval_mdd/mdd_result.sh \
            eval_mdd/human_seq \
            eval_mdd/ref \
            $exp_root/decode_$test/hyp_nosil \
            $exp_root/decode_$test

        echo
        echo -e "${GREEN}Write mdd result to $exp_root/decode_$test/mdd_result.txt${NC}"
        echo
        cat $exp_root/decode_$test/mdd_result.txt
        echo
