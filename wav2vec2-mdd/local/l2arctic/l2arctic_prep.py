...
path = args.l2_path+"/*/" + anno_dir + "/*.TextGrid"
...
w5 = open(tmp_path+"/phn_dur",'w+')
w6 = open(tmp_path+"/transcript_phn_dur",'w+')
w7 = open(tmp_path+"/phn_start",'w+')
w8 = open(tmp_path+"/transcript_phn_start",'w+')

def del_repeat_sil(phn_lst, dur_lst, start_lst):
    tmp = [phn_lst[0]]
    tmp_dur = [str(dur_lst[0])]
    tmp_start = [str(start_lst[0])]
    ...
    for i in range(1,len(phn_lst)):
        if(phn_lst[i] == phn_lst[i-1] and phn_lst[i]=="sil"):
            ...
            tmp_dur.append(str(dur_lst[i]))
            tmp_start.append(str(start_lst[i]))

    return tmp, tmp_dur, tmp_start
...
for phn_path in wav_lst:
    ...
    tmp = re.sub(anno_dir, "wav", phn_path)
    wav_path = re.sub("TextGrid", "wav",tmp)
    tmp = re.sub(anno_dir, "transcript",phn_path)
    ...
    if(spk_id in eval(type_[0]+"_spk") ):
        ...
        transcript_phns = []
        transcript_phns_dur = []
        transcript_phns_start = []
        #detections = [] # NOTE: detection labels
        tg = textgrid.TextGrid.fromFile(phn_path)
        
        for idx, i in enumerate(tg[1]):
            
            dur = (float(i.maxTime) - float(i.minTime)) * 100
            dur = int(dur)
            transcript_phns_dur.append(dur)
            transcript_phns_start.append(int(float(i.minTime) * 100))
        ...
        cur_phns, cur_phns_dur, cur_phns_start = del_repeat_sil(cur_phns, transcript_phns_dur, transcript_phns_start)
        transcript_phns, transcript_phns_dur, transcript_phns_start = del_repeat_sil(transcript_phns, transcript_phns_dur, transcript_phns_start)

        w3.write(utt_id + " " + " ".join(cur_phns) + "\n" )
        w4.write(utt_id + " " + " ".join(transcript_phns) + "\n" )
        w5.write(utt_id + " " + " ".join(cur_phns_dur) + "\n" )
        w6.write(utt_id + " " + " ".join(transcript_phns_dur) + "\n" )
        w7.write(utt_id + " " + " ".join(cur_phns_start) + "\n" )
        w8.write(utt_id + " " + " ".join(transcript_phns_start) + "\n" )
