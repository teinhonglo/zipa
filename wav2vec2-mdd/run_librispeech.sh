...
train=train_960_cleaned_val30s
dev=dev_clean
tests="dev_clean dev_other test_clean test_other"

# training config
nj=4
gpuid=1
seed=66 #824
train_conf=conf/train_librispeech_baseline_wav2vec2_large_lv60.json  # dev: 7.3 / test: 8.4
suffix=
...
exp_root=exp/librispeech/${conf_tag}${suffix}

if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then

    echo -e "${GREEN}Stage 0: data preparation ...${NC}"

    local/librispeech/data_prep.sh \
        --stage 0 \
        --librispeech-dir $librispeech_dir --phoneme-map $phoneme_map --map-file $map_file \
